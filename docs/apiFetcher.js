define([], function () {
  ...
  // ═══════════════════════ LIFECYCLE ═══════════════════════
  ApiFetcher.prototype.initialize = function (oControlHost, fnDoneInitializing) {
  ...
  ApiFetcher.prototype.setData = function (oControlHost, oDataStore) {
  ...
  ApiFetcher.prototype._extractSessionData = function (ds) {
    this.sessionData = {};
  ...
  ApiFetcher.prototype.draw = function (oControlHost) {
    this.m_oControlHost = oControlHost;
    this.config = oControlHost.configuration || {};
  ...
  ApiFetcher.prototype._applyConfigDefaults = function () {
    ...
    if (op) {
      ...
      if (op.redirectUri) this._setField("af-oauth-redirect", op.redirectUri);
    }
    ...
    if (this.config.callbackScriptUrl) {
      console.log("[ApiFetcher] callbackScriptUrl configured:", this.config.callbackScriptUrl);
    } else {
      console.log("[ApiFetcher] No callbackScriptUrl — default callback handler will be used");
    }
  };

  // ═══════════════════════ BUILD HTML ═══════════════════════
  ApiFetcher.prototype._buildHTML = function () {
    return [
  ...
  // ═══════════════════════ RENDER SESSION ═══════════════════════
  ApiFetcher.prototype._renderSessionInfo = function () {
  ...
  // ═══════════════════════ BIND EVENTS ═══════════════════════
  ApiFetcher.prototype._bindEvents = function () {
    var self = this,
      R = this.domNode;
  ...
  // ═══════════════════════ FFP PRESET FILL ═══════════════════════
  ApiFetcher.prototype._ffpFill = function () {
    var fp = this.config.ffpPreset;
    if (!fp || !fp.baseUrls) {
    ...
    var authType = this.domNode.querySelector("#af-auth-type");
    authType.value = authMode === "cookie" ? "cookie" : "bearer";
    authType.dispatchEvent(new Event("change"));
    if (fp.acceptHeader) this._setField("af-headers", "Accept: " + fp.acceptHeader);
    this._setSt("Preset filled for " + env + " (" + authMode + ")", "success");
    console.log("[ApiFetcher] FFP preset filled:", url);
  };

  // ═══════════════════════ OAUTH2 PKCE ═══════════════════════
  ApiFetcher.prototype._testPopup = function () {
  ...
  ApiFetcher.prototype._startOAuth = function () {
    var self = this;
    var authUrl = this._gv("af-oauth-auth-url"),
      clientId = this._gv("af-oauth-client-id"),
      redirect = this._gv("af-oauth-redirect"),
      scope = this._gv("af-oauth-scope"),
      method = this._gv("af-oauth-challenge");
    var st = this.domNode.querySelector("#af-oauth-status");
    ...
    this._genPKCE(method)
      .then(function (pkce) {
        ...
        // ── Build state: encode nonce + optional callbackScriptUrl ──────────
        var nonce = self._rnd(32);
        self.oauthNonce = nonce;

        var statePayload = { nonce: nonce };
        if (self.config.callbackScriptUrl) {
          statePayload.scriptUrl = self.config.callbackScriptUrl;
          // ── Pass everything the debug script needs to do token exchange ──
          statePayload.tokenUrl = self._gv("af-oauth-token-url");
          statePayload.clientId = clientId;
          statePayload.redirectUri = redirect;
          statePayload.codeVerifier = pkce.verifier;
          console.log("[OAuth] Using callbackScriptUrl:", self.config.callbackScriptUrl);
        }
  ...
  ApiFetcher.prototype._onOAuthMessage = function (ev) {
    ...
    if (ev.data.code) {
      if (this.oauthNonce && ev.data.state && ev.data.state !== this.oauthNonce) {
        ...
        st.className = "af-status af-status-error";
        console.warn("[OAuth] State mismatch — expected nonce:", this.oauthNonce, "received:", ev.data.state);
        return;
      }
    ...
    // ── Handle token posted directly from debug-callback.js ──────────────
  ...
  ApiFetcher.prototype._exchangeToken = function (code) {
    var self = this,
  ...
  ApiFetcher.prototype._useToken = function () {
  ...
  ApiFetcher.prototype._clearToken = function () {
    this.oauthToken = null;
    this.oauthTokenExpiry = null;
    this.codeVerifier = null;
    this.oauthState = null;
  ...
  // PKCE helpers
  ApiFetcher.prototype._genPKCE = function (m) {
    var v = this._rnd(64);
  ...
  ApiFetcher.prototype._rnd = function (n) {
  ...
  // ═══════════════════════ FETCHER ═══════════════════════
  ApiFetcher.prototype._doFetch = function () {
    var self = this,
    ...
    var method = this._gv("af-method"),
      ...
      hdrsRaw = this._gv("af-headers").trim(),
      bodyRaw = this._gv("af-body").trim();
    var headers = {};
    if (authType === "bearer" && authVal) headers["Authorization"] = "Bearer " + authVal;
    else if (authType === "cookie" && authVal) headers["Cookie"] = authVal;
    if (hdrsRaw) {
      hdrsRaw.split("\n").forEach(function (l) {
        var ci = l.indexOf(":");
  ...
  ApiFetcher.prototype._doPreflight = function () {
    var self = this,
      url = this._gv("af-url").trim();
    if (!url) {
  ...
  ApiFetcher.prototype._doDiscover = function () {
    var self = this,
      url = this._gv("af-url").trim();
    if (!url) {
      this._setSt("Enter URL", "error");
  ...
  // ═══════════════════════ DISPLAY ═══════════════════════
  ApiFetcher.prototype._showResults = function (m) {
    this._showCORS(m);
    this._showHdrs(m);
    this._showBody(m);
  ...
  ApiFetcher.prototype._showCORS = function (m) {
  ...
  ApiFetcher.prototype._showHdrs = function (m) {
  ...
  ApiFetcher.prototype._showBody = function (m) {
  ...
  ApiFetcher.prototype._showErr = function (m) {
    this._showCORS(m);
  ...
  // ═══════════════════════ HISTORY ═══════════════════════
  ApiFetcher.prototype._addHist = function (m) {
  ...
  // ═══════════════════════ HELPERS ═══════════════════════
  ApiFetcher.prototype._tog = function (bId, tId) {
  ...
  ApiFetcher.prototype._switchTab = function (t) {
  ...
  ApiFetcher.prototype._setSt = function (msg, type) {
    var el = this.domNode.querySelector("#af-status");
    el.textContent = msg;
    el.className = "af-status af-status-" + type;
  };
  ApiFetcher.prototype._clearResults = function () {
  ...
  ApiFetcher.prototype._gv = function (id) {
    var el = this.domNode.querySelector("#" + id);
    return el ? el.value : "";
  };
  ApiFetcher.prototype._setField = function (id, val) {
    var el = this.domNode.querySelector("#" + id);
    if (el) el.value = val;
  };
  ApiFetcher.prototype._fB = function (b) {
    if (b == null) return "N/A";
    if (b < 1024) return b + " B";
    if (b < 1048576) return (b / 1024).toFixed(1) + " KB";
    return (b / 1048576).toFixed(2) + " MB";
  };
  ApiFetcher.prototype._esc = function (s) {
  ...
  // ═══════════════════════ COGNOS LIFECYCLE ═══════════════════════
  ApiFetcher.prototype.getParameters = function () {
    return null;
  };
  ApiFetcher.prototype.isInValidState = function () {
    return true;
  };
  ApiFetcher.prototype.destroy = function () {
