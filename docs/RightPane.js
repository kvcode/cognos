define([], function () {
  ...
  // ADD CARD
  // ===========================================================================
  RightPane.prototype.addCard = function (config) {
    console.log(`[RightPane]  addCard() called for:`, config.label);

    if (!config || !config.label) {
      console.error("[RightPane]  Invalid config - missing label");
      return null;
    }

    const existingCard = this.getCardByParameter(
      config.paramName || (config.paramNames ? config.paramNames.from : null),
    );

    if (existingCard) {
      console.warn(`[RightPane]  Card already exists for parameter:`, config.paramName || config.paramNames);
      this.cardsContainer.scrollTop = existingCard.cardElement.offsetTop - 20;
      existingCard.cardElement.style.animation = "none";
      setTimeout(() => {
        existingCard.cardElement.style.animation = "highlight-card 0.6s ease";
      }, 10);
      return existingCard;
    }

    const cardObject = this._createCardObject(config);
    const card = this._renderCard(cardObject);

    cardObject.cardElement = card;
    this.cards.push(cardObject);

    this.cardsContainer.appendChild(card);

    setTimeout(() => {
      this.cardsContainer.scrollTop = card.offsetTop - 20;
    }, 100);

    if (this.m_oControlHost) {
      try {
        this.m_oControlHost.valueChanged();
        console.log("[RightPane]  Cognos notified of card addition");
    ...
    console.log(`[RightPane]  Card added successfully. Total cards: ${this.cards.length}`);
    return cardObject;
  };
  ...
  // CREATE CARD OBJECT
  // ===========================================================================
  RightPane.prototype._createCardObject = function (config) {
    console.log("[RightPane]  Creating card object for:", config.label);
